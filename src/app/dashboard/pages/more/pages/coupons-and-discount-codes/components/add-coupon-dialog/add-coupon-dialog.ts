...
import { SelectOption, CouponPayload } from '../../models/coupons-and-discount-codes.model';
...
@Component({
  ...
  styleUrl: './add-coupon-dialog.scss',
})
export class AddCouponDialog extends BaseComponent implements OnInit {
  ...
  // Cross-field validator: expires_at must be after starts_at
  ...
  onGetPlans() {
    ...
    this.couponsService
      ...
      .subscribe({
        next: (res) => {
          // Adjust mapping to match your actual plans API response shape
  ...
  onGetUsers() {
    ...
    this.userDatabaseService
      ...
      .subscribe({
        next: (res) => {
          // Adjust mapping to match your actual users API response shape
    ...
    this.couponsService
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.isSubmitting = false;
          this.errorMessage = 'حدث خطأ أثناء إضافة الكوبون، حاول مرة أخرى';
        },
      });
  }
